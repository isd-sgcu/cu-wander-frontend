...
interface AuthContextValue {
    ...
    signUp: (authCred: { [key: string]: string | number }, redirect: string) => Promise<void>;
...
const AuthProvider = ({ children }: { children: ReactNode | ReactNode[] }) => {
    ...
    const logIn = async (authCred: AuthCredentials, redirect: string): Promise<void> => {
        try {
    ...
    const signUp = async (userData: { [key: string]: string | number }, redirect: string): Promise<void> => {
        try {
    ...
    return <AuthContext.Provider value={{ user, logIn, signUp, signOut, isLoggedIn }}>{children}</AuthContext.Provider>;
};
