...
export default function Coupon() {
    ...
    const { setShowModal, searchPhrase, setSearchPhrase } = useContext(CouponState);
    ...
    return (
        <IonPage>
            ...
            >
                <img src="assets/icon/search.svg" className="px-2.5" alt="search icon" />
                <input
                    type="text"
                    value={searchPhrase}
                    onChange={(e) => {
            ...
            <IonContent fullscreen className="bg-white">
                ...
                {coupons && loading ? (
                    <div className="text-center text-green-500 h-48 pt-20">loading...</div>
                ) : (
                    <>
                        {coupons.length == 0 ? (
                            <div className="text-center text-black h-48 pt-20">No coupons available</div>
                        ) : (
                            <div className="flex flex-col items-center bg-white font-noto pt-12 pb-6 space-y-2">
                                {coupons?.map((coupon, idx) => {
                                    if (searchPhrase !== "") {
                                        if (!coupon.title.toLowerCase().includes(searchPhrase) && !coupon.shop_title.toLowerCase().includes(searchPhrase)) {
                                            return null;
                                        }
                                    }
                                    ...
                                    [...Array(totalPages)].map((_, idx) => (
                                        <button
                                            ...
                                            className={clsx(
                                                ...
                                                page === idx + 1 ? "border border-dashed border-green-500/75 rounded-md text-green-500 font-bold" : "text-black/75 font-semibold"
                                            )}
                                        >
                                            {idx + 1}
                                        </button>
